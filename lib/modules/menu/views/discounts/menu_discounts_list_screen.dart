...
class MenuDiscountsListScreen extends StatelessWidget {
  const MenuDiscountsListScreen({super.key});

  @override
  Widget build(BuildContext context) {
    // Get the menu item from arguments
    final dynamic args = Get.arguments;
    final MenuItemModel? menuItem = args is MenuItemModel ? args : null;
    final int? menuId = menuItem?.id ?? (args is int ? args : null);

    if (menuId == null) {
      // Navigate back if no menu ID is provided
    ...
    return GetBuilder<DiscountController>(
      ...
      initState: (state) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          Get.find<DiscountController>().getDiscountsForMenuItem(menuId);
        });
      },
      builder: (controller) {
        return Scaffold(
          ...
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              controller.initializeCreateForm(menuId);
              Get.toNamed(Routes.ADD_DISCOUNT_SCREEN, arguments: menuId);
            },
          ...
          body: Container(
            ...
            child: RefreshIndicator(
              onRefresh: () async {
                await controller.getDiscountsForMenuItem(menuId);
              },
            ...
            children: [
              ...
              Expanded(
                child: CustomButton(
                  onPressed: () async {
                    Get.back();
                    await controller.deleteDiscount(menuId, discount.id);
                  },
                  title: "Delete",
